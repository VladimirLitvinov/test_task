...
from schemas.responses import ReferralResponseSchema, \
    CodeForUserResponseSchema, ReferralListResponseSchema
...
@router.post("/referral_code", status_code=200,
             response_model=CodeForUserResponseSchema,
             responses={404: {"model": NotFoundReferralCodeSchema}})
async def get_referral_code(email: UserSearchSchema,
                            session: AsyncSession = Depends(
                                get_async_session)):
...
@router.post("/register", status_code=201,
             response_model=ReferralResponseSchema)
async def register_referral(referral: CreateReferralSchema,
                            session: AsyncSession = Depends(
                                get_async_session)):
