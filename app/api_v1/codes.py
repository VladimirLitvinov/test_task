...
from schemas.exceptions import IntegrityErrorResponseSchema, \
    NotFoundReferralCodeSchema
from schemas.responses import ReferralCodeResponseSchema

router = APIRouter(tags=['codes'])


@router.post('/register', response_model=ReferralCodeResponseSchema,
             status_code=201,
             responses={409: {"model": IntegrityErrorResponseSchema}})
async def create_code(date: CreateCode,
