...
async def create_referral_code(session: AsyncSession,
                               user: UserSchema, date: date) -> ReferralCode:
    referral_code = ReferralCode(author_id=user.id, code=generate_code(),
                                 valid_until=date)
    ...
    try:
        ...
        raise CustomApiException(status_code=409,
                                 detail="У вас уже есть промокод")
    return referral_code


async def get_referral_code_by_email(session: AsyncSession,
                                     email: str) -> ReferralCode:
    query = select(User).options(joinedload(User.code_value)).where(
        User.email == email)
    user = await session.execute(query)
    result = user.unique().scalar_one_or_none()
    if not result:
        raise CustomApiException(status_code=404,
                                 detail="Пользователь с таким email не найден")
    if result.code_value is None:
        raise CustomApiException(status_code=404,
                                 detail="У пользователя ещё нет кода")

    return result.code_value


async def get_referral_code(session: AsyncSession,
                            referral_code: str) -> ReferralCode:
...
async def create_referral(session: AsyncSession, user_id: int,
                          code_id: int) -> Referral:
...
async def get_referrals(session: AsyncSession, user_id: int):
    query = select(ReferralCode).options(
        joinedload(ReferralCode.referrals).joinedload(Referral.user)).where(
        ReferralCode.author_id == user_id)
    ...
    if len(result.referrals) == 0:
        raise CustomApiException(status_code=404,
                                 detail="У пользователя ещё нет рефералов")
